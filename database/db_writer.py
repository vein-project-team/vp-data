...
from database.dispatcher import get_trade_date_list_from_api
from database.dispatcher import get_stock_list_from_api
from database.dispatcher import get_index_quotation_from_api, get_stocks_details_by_exchange_from_api
from database.dispatcher import get_up_down_limits_statistic_details_from_api
...
def fill_tables(size):
    if not dc.check_trade_date_list_table('DAILY'):
        write_to_db('TRADE_DATE_LIST_DAILY', get_trade_date_list_from_api('DAILY', size))
    if not dc.check_trade_date_list_table('WEEKLY'):
        write_to_db('TRADE_DATE_LIST_WEEKLY', get_trade_date_list_from_api('WEEKLY', size))
    if not dc.check_trade_date_list_table('MONTHLY'):
        write_to_db('TRADE_DATE_LIST_MONTHLY', get_trade_date_list_from_api('MONTHLY', size))

    if not dc.check_stock_list_table('SH'):
        write_to_db('SH_STOCK_LIST', get_stock_list_from_api('SH'))
    if not dc.check_stock_list_table('SZ'):
        write_to_db('SZ_STOCK_LIST', get_stock_list_from_api('SZ'))
    ...
    if not dc.check_details_table('SH', 'DAILY'):
        write_to_db('SH_DETAILS_DAILY', get_stocks_details_by_exchange_from_api('SH', 'DAILY', size))
    if not dc.check_details_table('SH', 'WEEKLY'):
        write_to_db('SH_DETAILS_WEEKLY', get_stocks_details_by_exchange_from_api('SH', 'WEEKLY', size))
    if not dc.check_details_table('SH', 'MONTHLY'):
        write_to_db('SH_DETAILS_MONTHLY', get_stocks_details_by_exchange_from_api('SH', 'MONTHLY', size))

    if not dc.check_details_table('SZ', 'DAILY'):
        write_to_db('SZ_DETAILS_DAILY', get_stocks_details_by_exchange_from_api('SZ', 'DAILY', size))
    if not dc.check_details_table('SZ', 'WEEKLY'):
        write_to_db('SZ_DETAILS_WEEKLY', get_stocks_details_by_exchange_from_api('SZ', 'WEEKLY', size))
    if not dc.check_details_table('SZ', 'MONTHLY'):
        write_to_db('SZ_DETAILS_MONTHLY', get_stocks_details_by_exchange_from_api('SZ', 'MONTHLY', size))

    if not dc.check_limits_statistic_details_table('SH'):
        write_to_db('SH_LIMITS_STATISTIC_DETAILS', get_up_down_limits_statistic_details_from_api(size, 'SH'))
    if not dc.check_limits_statistic_details_table('SZ'):
        write_to_db('SZ_LIMITS_STATISTIC_DETAILS', get_up_down_limits_statistic_details_from_api(size, 'SZ'))
