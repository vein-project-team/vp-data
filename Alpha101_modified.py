...
def Preparations_for_Alpha101_alpha048():          
    ...
    date_list=local_source.get_indices_daily(cols='TRADE_DATE',condition='INDEX_CODE = "000001.SH"')["TRADE_DATE"]  
    
    #check for building/updating/reading dataset
    try:
        result_indneutralized_df = pd.read_csv("Preparations_for_alpha048_indneutralized_data.csv").set_index("TRADE_DATE")
        date_list_existed = result_indneutralized_df.index
        date_list_update = [date for date in date_list if date not in date_list_existed]
        if len(date_list_update)==0:
            print("The corresponding indneutralized data needs not to be updated.")
            return result_indneutralized_df
        else:
            print("The corresponding indneutralized data needs to be updated.")
            first_date_update = date_list_update[0]
    except:
        print("The corresponding indneutralized dataset is missing.")
        result_indneutralized_df=pd.DataFrame(index=date_list,columns=industry_list)
        result_unindneutralized_industry=0 
    ...
    for industry in pb(industry_list, desc='Please wait', colour='#ffffff'):
        ...
        for ts_code in stock_list_industry.index:
            ...
            result_unindneutralized_piece = (correlation(delta(CLOSE, 1), delta(delay(CLOSE, 1), 1), 250) *delta(CLOSE, 1)) / CLOSE
                
            result_unindneutralized_piece.rename("PREPARATION_FOR_ALPHA048_UNINDNEUTRALIZED",inplace=True)
            result_unindneutralized_piece = pd.DataFrame(result_unindneutralized_piece)
            result_unindneutralized_piece.insert(loc=0,column='INDUSTRY',value=industry)
            result_unindneutralized_piece.insert(loc=0,column='TRADE_DATE',value=quotations_daily_chosen["TRADE_DATE"])
            result_unindneutralized_piece.insert(loc=0,column='TS_CODE',value=ts_code)
            
            try:
                result_unindneutralized_piece = result_unindneutralized_piece[result_unindneutralized_piece["TRADE_DATE"]>=first_date_update]
            except:
                pass
        ...
        for date in date_list_update:
            try:
                result_piece=result_unindneutralized_industry[result_unindneutralized_industry["TRADE_DATE"]==date]
                value=result_piece["PREPARATION_FOR_ALPHA048_UNINDNEUTRALIZED"].mean()
                result_indneutralized_df.loc[date,industry]=value
            except:
                pass
    
    result_indneutralized_df.to_csv("Preparations_for_alpha048_indneutralized_data.csv",encoding='utf-8-sig')           
    return result_indneutralized_df
    


class Alphas(object):
    def __init__(self, ts_code="000001.SZ"):
        ...
        self.available_dates = quotations_daily_chosen["TRADE_DATE"]
...
#       alpha = pd.DataFrame(np.ones_like(self.close), index=self.close.index, columns=self.close.columns)
    ...
    def alpha048(self):
        indneutralized_data = Preparations_for_Alpha101_alpha048()
        alpha = - 1 / sma(((delta(self.close, 1) / delay(self.close, 1))^2), 250)
        alpha.rename("0",inplace=True)
        alpha=pd.DataFrame(alpha)
        alpha.insert(loc=0,column='TRADE_DATE',value=self.available_dates)
        alpha.set_index('TRADE_DATE')
        for date in alpha.index:
            alpha.loc[date, "0"] = indneutralized_data.loc[date, self.industry] / alpha.loc[date, "0"]
        return alpha
...
'''
quotations_daily_piece=local_source.get_quotations_daily(cols='TRADE_DATE,TS_CODE,OPEN,CLOSE,LOW,HIGH,VOL,CHANGE,AMOUNT',condition='TS_CODE ="000001.SZ"').sort_values(by="TRADE_DATE" , ascending=True) 

stock_indicators_daily_piece=local_source.get_stock_indicators_daily(cols='TRADE_DATE,TS_CODE,TOTAL_SHARE',condition='TS_CODE ="000001.SZ"').sort_values(by="TRADE_DATE" , ascending=True) 
data_test=pd.merge(quotations_daily_piece,stock_indicators_daily_piece,on=['TRADE_DATE','TS_CODE'],how="left") 
data_test["TOTAL_MV"]=data_test["TOTAL_SHARE"]*data_test["CLOSE"]
alpha_test=Alphas(data_test)
#get_alpha(data_test)
#test=local_source.get_quotations_daily(cols='TRADE_DATE,TS_CODE,OPEN,CLOSE,LOW,HIGH,VOL,CHANGE,AMOUNT').sort_values(by="TRADE_DATE" , ascending=True) 
'''   