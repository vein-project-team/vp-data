...
class Alphas(object):
    def __init__(self, ts_code="000001.SZ"):
        ...
        self.available_dates = quotations_daily_chosen["TRADE_DATE"].astype(int)
    ...
    def alpha092(self):
        ...
        df.at[df['p2']>=df['p1'],'min']=df['p1']
        alpha = df['min']
        #alpha = min(ts_rank(decay_linear(((((self.high + self.low) / 2) + self.close) < (self.low + self.open)).to_frame(), 15).CLOSE,19), ts_rank(decay_linear(correlation(rank(self.low), rank(adv30), 8).to_frame(), 7).CLOSE,7))
        return alpha
